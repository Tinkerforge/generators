 ...
 *
 * Patched to use hal_log_error instead of writing to stderr
 */
...
void bcm2835_spi_transfernb(const char* tbuf, char* rbuf, uint32_t len)
{
    volatile uint32_t* paddr = bcm2835_spi0 + BCM2835_SPI0_CS/4;
    volatile uint32_t* fifo = bcm2835_spi0 + BCM2835_SPI0_FIFO/4;
    uint32_t TXCnt=0;
...
static void *mapmem(const char *msg, size_t size, int fd, off_t off)
{
    ...
    if (map == MAP_FAILED)
	    tf_hal_log_error("bcm2835_init: %s mmap failed: %s\n", msg, strerror(errno));
    return map;
}
...
int bcm2835_init(void)
{
    ...
    */
	...
	{
	  tf_hal_log_error("bcm2835_init: Unable to open /dev/mem: %s\n", strerror(errno));
	  goto exit;
	}
	...
	{
	  tf_hal_log_error("bcm2835_init: Unable to open /dev/gpiomem: %s\n", strerror(errno));
	  goto exit;
	}
