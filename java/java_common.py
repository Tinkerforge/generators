...
class JavaElement(common.Element):
    ...
    def get_java_type(self, cardinality=None):
        ...
        return get_java_type(self.get_type(), cardinality,
                             legacy=self.get_device().has_java_legacy_types() or is_virtual or self.get_packet().get_function_id() == 255,
...
def detect_java_home():
    for java_home in ['/usr/lib/jvm/java-21-openjdk-amd64', '/usr/lib/jvm/java-21-openjdk']:
        ...
        if javac_version.startswith('javac 21'):
            return java_home

    raise Exception('Could not detect JAVA_HOME for Java 21')
