...
"""
JuliaPy Debian Package Generator
Copyright (C) 2020 Matthias Bolte <matthias@tinkerforge.com>

generate_juliapy_debian_package.py: Generator for JuliaPy Debian Package
...
def generate(root_dir, language):
    ...
    tmp_build_dir         = os.path.join(tmp_dir, 'tinkerforge-juliapy-bindings-{0}.{1}.{2}'.format(*version))
    ...
    common.execute(['unzip',
                    '-q',
                    os.path.join(root_dir, 'tinkerforge_juliapy_bindings_{0}_{1}_{2}.zip'.format(*version)),
                    os.path.join('source', '*'),
                    '-d',
                    tmp_dir])
...
if __name__ == '__main__':
    common.dockerize('juliapy', __file__)
