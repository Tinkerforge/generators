...
generate_python_debian_package.py: Generator for Python Debian Package
...
def generate(root_dir):
    ...
    tmp_build_dir         = os.path.join(tmp_dir, 'tinkerforge-python-bindings-{0}.{1}.{2}'.format(*version))
    ...
    common.execute(['unzip',
                    '-q',
                    os.path.join(root_dir, 'tinkerforge_python_bindings_{0}_{1}_{2}.zip'.format(*version)),
                    os.path.join('source', '*'),
                    '-d',
                    tmp_dir])
