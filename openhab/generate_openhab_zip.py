...
class OpenHABZipGenerator(openhab_common.OpenHABGeneratorTrait, common.ZipGenerator):
    ...
    def finish(self):
        ...
        with common.ChangedDirectory(binding_dir):
            #common.execute(['mvn', 'clean', 'spotless:apply', '-Dorg.slf4j.simpleLogger.defaultLogLevel=WARN'])
            common.execute(['mvn', 'clean', 'install', '-DskipChecks', '-DskipTests', '-Dspotless.check.skip=true', '-Dorg.slf4j.simpleLogger.defaultLogLevel=WARN'])
        ...
        shutil.copy(os.path.join(binding_dir, 'target', 'org.openhab.binding.tinkerforge-2.5.9.jar'), zip_dir)

        java_bindings = os.path.join(self.get_root_dir(), 'tinkerforge-2.1.31-unreleased.jar')
