...
/**
 * The {@link BrickletOutdoorWeatherSensorHandler} is responsible for handling
 * commands, which are sent to one of the channels.
 *
 * @author Erik Fleckstein - Initial contribution
 */
@NonNullByDefault
public class BrickletOutdoorWeatherSensorHandler extends BaseThingHandler {
    private final Logger logger = LoggerFactory.getLogger(BrickletOutdoorWeatherSensorHandler.class);

    private @Nullable BrickletOutdoorWeatherSensor device;
    ...
    public BrickletOutdoorWeatherSensorHandler(Thing thing,
    ...
    public void initialize() {
        ...
        if (device != null) {
            Utils.assertNonNull(outdoorWeatherHandler.getDevice()).removeSensorDataListener(Utils.assertNonNull(device).listener);
    ...
    private void initializeDevice() {
        ...
        BrickletOutdoorWeatherSensor dev = device;
        if (dev != null)
            Utils.assertNonNull(outdoorWeatherHandler.getDevice()).removeSensorDataListener(dev.listener);

        dev = new BrickletOutdoorWeatherSensor(Utils.assertNonNull(outdoorWeatherHandler.getDevice()));
        this.device = dev;
    ...
    private void refreshValue(String channelId, Configuration channelConfig) {
        try {
            @Nullable BrickletOutdoorWeatherSensor dev = this.device;
            if (dev == null) {
                return;
            }
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        ...
        try {
            if (command instanceof RefreshType) {
                ...
                @Nullable BrickletOutdoorWeatherSensor dev = device;
                if (dev == null) {
                    return;
                }
