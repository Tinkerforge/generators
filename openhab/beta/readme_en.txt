...
Configuration:
    ...
    Brick Daemon instance. Devices that are not supported (yet) are marked
    in their description. To make keeping track easier, these devices can
    be hidden.
...
Further documentation:
    Can be found in the doc subfolder of the bindings. Supported configuration
    parameters and channels are listed there per device.
...
Actions in rules:
    ...
    The parameters and return values correspond to those of the respective
    function in the Java Bindings
    https://www.tinkerforge.com/ed/doc/Software/API_Bindings_Java.html
    Functions in the Java Bindings that expect arrays as parameters can also
    ...
    Nearly the complete API of devices can be used as actions. Excluded are
    only operations, that can invalidate the state of channels. You can use items
    associated to the channels with .sendCommand to change those. Also not
    supported are operations, that would write EEPROM or Flash Storage, to
    avoid unneccesary write-cycles.
    
    The available actions per device are listed in the doc subfolder of the
    bindings. The names correspond to those of the Java Bindings, where further
...
Known Bugs:    
    ...
    Removing non-attached devices sometimes doesn't work: The device handler
    tries to clean up be deactivating callbacks, if the device can not be reached,
    this throws TimeoutExceptions that are currently not handled.
    
    The generated code must be improved: Compiling raises a lot of warnings,
    Listener(de)registrations can be duplicated.
