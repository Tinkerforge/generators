...
generate_micropython_bindings.py: Generator for MicroPython bindings
...
class MicroPythonBindingsDevice(micropython_common.MicroPythonDevice):
    def get_micropython_import(self):
...
"""
    ...
    def get_micropython_namedtuples(self):
        tuples = ''
...
"""
    ...
    def get_micropython_class(self):
        template = """
class {0}(Device):
    r\"\"\"
    {1}
...
"""
    ...
    def get_micropython_callback_id_definitions(self):
        callback_ids = ''
    ...
    def get_micropython_function_id_definitions(self):
        function_ids = '\n'
    ...
    def get_micropython_constants(self):
    ...
    def get_micropython_init_method(self):
        template = """
...
"""
        ...
        for packet in self.get_packets('function'):
            response_expected += '        self.response_expected[{0}.FUNCTION_{1}] = {0}.RESPONSE_EXPECTED_{2}\n' \
                                 .format(self.get_micropython_class_name(), packet.get_name().upper,
                                         packet.get_response_expected().upper())

        return template.format(self.get_micropython_class_name(), *self.get_api_version()) + common.wrap_non_empty('', response_expected, '\n')

    def get_micropython_callback_formats(self):
        callback_formats = ''
        template = "        self.callback_formats[{0}.CALLBACK_{1}] = ({2}, '{3}')\n"

        for packet in self.get_packets('callback'):
            callback_formats += template.format(self.get_micropython_class_name(),
                                                packet.get_name().upper,
                                                packet.get_response_size(),
                                                packet.get_micropython_format_list('out'))

        return callback_formats + '\n'

    def get_micropython_high_level_callbacks(self):
        ...
        for packet in self.get_packets('callback'):
            ...
            if stream != None:
                ...
                high_level_callbacks += template.format(self.get_micropython_class_name(),
    ...
    def get_micropython_add_device(self):
        return '        ipcon.add_device(self)\n'

    def get_micropython_methods(self):
        m_tup = """
    def {0}(self{8}{4}):
        r\"\"\"
        {10}
...
"""
        ...
        for packet in self.get_packets('function'):
            ...
            nh = ns.upper()
            par = packet.get_micropython_parameters()
            doc = packet.get_micropython_formatted_doc()
            cp = ''
            ct = ''
            ...
            in_f = packet.get_micropython_format_list('in')
            out_l = packet.get_response_size()
            out_f = packet.get_micropython_format_list('out')
...
"""
    ...
    def {function_name}(self{high_level_parameters}):
        ...
        ret = self.{function_name}_low_level({parameters}){dynamic_length_2}
...
"""
        ...
        for packet in self.get_packets('function'):
            ...
            if stream_in != None:
                    ...
                    if len(packet.get_elements(direction='out', high_level=True)) < 2:
                        if stream_in.has_single_chunk():
                            result = template_stream_in_single_chunk_result.format(function_name=packet.get_name(skip=-2).under,
                                                                                   parameters=packet.get_micropython_parameters())
                        else:
                ...
                methods += template.format(doc=packet.get_micropython_formatted_doc(),
                                           coercions=common.wrap_non_empty('\n        ', packet.get_micropython_parameter_coercions(high_level=True), '\n'),
                                           function_name=packet.get_name(skip=-2).under,
                                           parameters=packet.get_micropython_parameters(),
                                           high_level_parameters=common.wrap_non_empty(', ', packet.get_micropython_parameters(high_level=True), ''),
                                           ...
                                           chunk_padding=stream_in.get_chunk_data_element().get_micropython_default_item_value(),
    ...
    def get_micropython_register_callback_method(self):
...
"""
    ...
    def get_micropython_old_name(self):
        template = """
{0} = {1} # for backward compatibility
"""

        return template.format(self.get_name().camel, self.get_micropython_class_name())

    def get_micropython_source(self):
        source  = self.get_micropython_import()
        source += self.get_micropython_namedtuples()
        source += self.get_micropython_class()
        source += self.get_micropython_callback_id_definitions()
        source += self.get_micropython_function_id_definitions()
        source += self.get_micropython_constants()
        source += self.get_micropython_init_method()
        source += self.get_micropython_callback_formats()
        source += self.get_micropython_high_level_callbacks()
        source += self.get_micropython_add_device()
        source += self.get_micropython_methods()
        source += self.get_micropython_register_callback_method()

        if self.is_brick() or self.is_bricklet():
            source += self.get_micropython_old_name()

        return common.strip_trailing_whitespace(source)

class MicroPythonBindingsPacket(micropython_common.MicroPythonPacket):
    ...
    def get_micropython_format_list(self, io):
        forms = []

        for element in self.get_elements(direction=io):
            forms.append(element.get_micropython_struct_format())

        return ' '.join(forms)

    def get_micropython_parameter_coercions(self, high_level=False):
        coercions = []

        for element in self.get_elements(direction='in', high_level=high_level):
            name = element.get_name().under

            coercions.append('{0} = {1}'.format(name, element.get_micropython_parameter_coercion().format(name)))

        return '\n        '.join(coercions)

class MicroPythonBindingsGenerator(micropython_common.MicroPythonGeneratorTrait, common.BindingsGenerator):
    def get_device_class(self):
        return MicroPythonBindingsDevice

    def get_packet_class(self):
        return MicroPythonBindingsPacket
    ...
    def generate(self, device):
        ...
        with open(os.path.join(self.get_bindings_dir(), filename), 'w') as f:
            f.write(device.get_micropython_source())

        self.device_factory_all_classes.append((device.get_micropython_import_name(), device.get_micropython_class_name()))

        if device.is_released():
            self.device_factory_released_classes.append((device.get_micropython_import_name(), device.get_micropython_class_name()))
            self.released_files.append(filename)

    def finish(self):
        template_import = """from {0} import {1}
"""
        template = """# -*- coding: utf-8 -*-
{0}
{1}
...
def generate(root_dir, language, internal):
    common.generate(root_dir, language, internal, MicroPythonBindingsGenerator)
