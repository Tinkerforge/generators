...
"""
MicroPython Documentation Generator
Created by René Rohner
Copyright (C) 2026 Tinkerforge GmbH

generate_micropython_doc.py: Generator for MicroPython documentation
...
class MicroPythonDocDevice(micropython_common.MicroPythonDevice):
    def specialize_micropython_doc_function_links(self, text):
        def specializer(packet, high_level):
            if packet.get_type() == 'callback':
                return ':py:attr:`CALLBACK_{1} <{0}.CALLBACK_{1}>`'.format(packet.get_device().get_micropython_class_name(),
                                                                           packet.get_name(skip=-2 if high_level else 0).upper)
            else:
                return ':py:func:`{1}() <{0}.{1}>`'.format(packet.get_device().get_micropython_class_name(),
                                                           packet.get_name(skip=-2 if high_level else 0).under)

        return self.specialize_doc_rst_links(text, specializer, prefix='py')

    def get_micropython_examples(self):
    ...
    def get_micropython_functions(self, type_):
        functions = []
        template = '.. py:function:: {0}.{1}({2})\n\n{3}{4}\n'
        cls = self.get_micropython_class_name()
        ...
        for packet in self.get_packets('function'):
            ...
            params = packet.get_micropython_parameters(high_level=True)
            meta = packet.get_formatted_element_meta(lambda element, cardinality=None: element.get_micropython_type(cardinality=cardinality),
                                                     lambda element, index=None: element.get_micropython_name(index=index),
            ...
            desc = packet.get_micropython_formatted_doc()

            functions.append(template.format(cls, name, params, meta_table, desc))

        return ''.join(functions)

    def get_micropython_callbacks(self):
        callbacks = []
...
{3}
 ...
 :ref:`below <{0}_micropython_callbacks>`.
""",
            'de': """
.. py:function:: {2}{1}.register_callback(callback_id, function)
...
{3}
 ...
 :ref:`unten <{0}_micropython_callbacks>` zu finden.
"""
        }

        c_str = {
            'en': """
.. _{0}_micropython_callbacks:
...
.. note::
 Because MicroPython doesn't support threads, you need to call
 :py:func:`dispatch_callbacks() <IPConnection.dispatch_callbacks>` periodically
 to ensure that incoming callbacks are handled.
...
.. note::
 Da MicroPython keine Threads unterstützt, muss
 :py:func:`dispatch_callbacks() <IPConnection.dispatch_callbacks>` periodisch
 aufgerufen werden, damit eingehende Callbacks verarbeitet werden.
...
"""
        ...
        api = {
            'en': """
.. _{0}_micropython_api:

API
---

Generally, every function of the MicroPython bindings can throw an
``ip_connection.Error`` exception that has a ``value`` and a
...
Because MicroPython doesn't support threads, none of the functions listed below
are thread-safe. The MicroPython bindings use synchronous I/O. Use
:py:func:`dispatch_callbacks() <IPConnection.dispatch_callbacks>` to handle
callbacks.
...
Prinzipiell kann jede Funktion der MicroPython Bindings
``ip_connection.Error`` Exception werfen, welche ein ``value`` und
...
Da MicroPython keine Threads unterstützt, ist keine der folgend aufgelisteten
Funktionen Thread-sicher. Die MicroPython Bindings verwenden synchrone I/O.
Verwende :py:func:`dispatch_callbacks() <IPConnection.dispatch_callbacks>` um
...
"""
        ...
        create_meta = common.format_simple_element_meta([('uid', 'str', 1, 'in'),
                                                         ...
                                                         (self.get_name().under, self.get_micropython_class_name(), 1, 'out')])
        create_meta_table = common.make_rst_meta_table(create_meta)

        cre = common.select_lang(create_str).format(self.get_micropython_class_name(),
                                                    self.get_name().under,
                                                    create_meta_table)
        ...
        bf = self.get_micropython_functions('bf')
        af = self.get_micropython_functions('af')
        ccf = self.get_micropython_functions('ccf')
        c = self.get_micropython_callbacks()
        vf = self.get_micropython_functions('vf')
        if_ = self.get_micropython_functions('if')
        api_str = ''
        ...
        if c:
            ...
            api_str += common.select_lang(c_str).format(self.get_doc_rst_ref_name(),
                                                        self.get_micropython_class_name(),
                                                        self.get_name().under,
                                                        c)
        ...
        return common.select_lang(api).format(self.get_doc_rst_ref_name(),
                                              self.specialize_micropython_doc_function_links(common.select_lang(self.get_doc())),
                                              api_str)

    def get_micropython_doc(self):
        doc  = common.make_rst_header(self)
        doc += common.make_rst_summary(self)
        doc += self.get_micropython_examples()
        doc += self.get_micropython_api()

        return doc

class MicroPythonDocPacket(micropython_common.MicroPythonPacket):
    def get_micropython_formatted_doc(self):
        text = common.select_lang(self.get_doc_text())
        text = self.get_device().specialize_micropython_doc_function_links(text)
        ...
        prefix = self.get_device().get_micropython_class_name() + '.'
...
class MicroPythonDocGenerator(micropython_common.MicroPythonGeneratorTrait, common.DocGenerator):
    ...
    def get_device_class(self):
        return MicroPythonDocDevice

    def get_packet_class(self):
        return MicroPythonDocPacket
    ...
    def generate(self, device):
        with open(device.get_doc_rst_path(), 'w') as f:
            f.write(device.get_micropython_doc())

def generate(root_dir, language, internal):
    common.generate(root_dir, language, internal, MicroPythonDocGenerator)
