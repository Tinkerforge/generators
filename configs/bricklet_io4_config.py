...
def input_channel(idx):
    return {
            ...
            'getters': [{
                'packet': 'Get Value',
                'element': 'Value Mask',
                'transform': '(value & (1 << {})) > 0 ? OpenClosedType.OPEN : OpenClosedType.CLOSED'.format(idx)}],

            'callbacks': [{
                'filter': '(interruptMask & (1 << {})) > 0'.format(idx),
                'packet': 'Interrupt',
                'element': 'Value Mask',
                'transform': '(valueMask & (1 << {})) > 0 ? OpenClosedType.OPEN : OpenClosedType.CLOSED'.format(idx)}],
...
com['openhab'] = {
    ...
    'channels': channels,
    'channel_types': [
        oh_generic_channel_type('Input', 'Contact', 'NOT USED',
                    update_style=None,
    ...
    'actions': [{'fn': 'Set Value', 'refreshs': ['Output 0', 'Output 1', 'Output 2', 'Output 3', 'Monoflop 0', 'Monoflop 1', 'Monoflop 2', 'Monoflop 3']},
                ...
                {'fn': 'Get Edge Count', 'refreshs': ['Edge Count 0', 'Edge Count 1', 'Edge Count 2', 'Edge Count 3']},
