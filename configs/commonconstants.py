...
def add_callback_value_function(packets, name, data_name, data_type, doc, channel_based = False, since_firmware = [1, 0, 0]):
    name_get = name
    name_set = name.replace('Get ', 'Set ')
    name = name.replace('Get ', '')

    # Not on channels
    if not channel_based:
        getter = {
            'type': 'function',
            'name': name_get,
...
""".format(name_set, name_get)
    ...
    # On channels
        ...
        getter = {
            'type': 'function',
            'name': name_get,
            'elements': [('Channels', 'uint8', 1, 'in'),
                         (data_name, data_type, 1, 'out')],
            'since_firmware': since_firmware,
            'doc': ['bf', doc]
        }
