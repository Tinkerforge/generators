...
"""
MQTT Debian Package Generator
Copyright (C) 2020 Matthias Bolte <matthias@tinkerforge.com>

generate_mqtt_debian_package.py: Generator for MQTT Debian Package
...
def generate(root_dir):
    ...
    common.execute(['unzip',
                    '-q',
                    os.path.join(root_dir, 'tinkerforge_mqtt_bindings_{0}_{1}_{2}.zip'.format(*version)),
                    '-d',
                    tmp_dir])
    ...
    shutil.move(os.path.join(tmp_dir, 'tinkerforge-mqtt_{0}.{1}.{2}_all.deb'.format(*version)),
                os.path.join(root_dir, 'tinkerforge-mqtt_{0}.{1}.{2}_all.deb'.format(*version)))
